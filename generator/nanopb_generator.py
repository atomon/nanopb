...
class EncodedSize:
    ...
    def __add__(self, other):
        if isinstance(other, int):
            ...
            return EncodedSize(
                self.value,
                self.symbols + [str(other)],
                self.declarations,
                self.required_defines + [str(other)],
            )
            ...
            return EncodedSize(
                ...
                self.required_defines + other.required_defines,
            )
        else:
...
class Enum(ProtoElement):
    ...
    def __str__(self):
        ...
        if (FieldD.TYPE_ENUM, self.options.enum_intsize) in datatypes:
            self.ctype, self.pbtype, self.enc_size, self.data_item_size = datatypes[
                (FieldD.TYPE_ENUM, self.options.enum_intsize)
            ]
            result += ": " + self.ctype
        ...
        for index, (name, value) in enumerate(self.values):
            ...
            enum_value = "    %s = %d%s" % (
                Globals.naming_style.enum_entry(name),
                value,
                comma,
            )
            if trailing_comment:
    ...
    def auxiliary_defines(self):
        ...
        if unmangledName:
            ...
            self.protofile.manglenames.reverse_name_mapping[identifier] = (
                unmangledIdentifier
            )
        ...
        if not self.options.long_names:
            ...
            for i, x in enumerate(self.values):
                result += "#define %s %s\n" % (
                    Globals.naming_style.define_name(self.value_longnames[i]),
                    Globals.naming_style.enum_entry(x[0]),
                )
...
class Field(ProtoElement):
    ...
    def __init__(
        self, struct_name, desc, field_options, element_path=(), comments=None
    ):
        ...
        if desc.label == FieldD.LABEL_REPEATED:
            ...
            if (
                desc.type == FieldD.TYPE_MESSAGE
                and not field_options.proto3_singular_msgs
            ):
        ...
        if field_options.type == nanopb_pb2.FT_STATIC and not self.can_be_static:
            raise Exception(
                "Field '%s' is defined as static, but max_size or "
                "max_count is not given." % self.name
            )

        if field_options.fixed_count and self.max_count is None:
            raise Exception(
                "Field '%s' is defined as fixed count, "
        ...
        if desc.type in datatypes:
            ...
            if (desc.type, field_options.int_size) in datatypes:
                self.ctype, self.pbtype, self.enc_size, self.data_item_size = datatypes[
                    (desc.type, field_options.int_size)
                ]
            ...
            if field_options.fixed_length:
                ...
                if self.max_size is None:
                    raise Exception(
                        "Field '%s' is defined as fixed length, "
                ...
                if self.allocation == "STATIC":
                    self.ctype = Globals.naming_style.bytes_type(
            ...
            self.enc_size = (
                None  # Needs to be filled in after the message type is available
            )
    ...
    def __str__(self):
        ...
        type_name = (
            Globals.naming_style.type_name(self.ctype)
        ...
        if self.allocation == "POINTER":
            ...
            if self.rules == "FIXARRAY" and self.pbtype in ["STRING", "BYTES"]:
                ...
                result += "    struct %s *%s;" % (
                    Globals.naming_style.struct_name(self.ctype),
                    var_name,
                )
            else:
    ...
    def data_size(self, dependencies):
        ...
        if self.allocation == "POINTER" or self.pbtype == "EXTENSION":
            ...
            if str(self.submsgname) in dependencies:
                ...
                sys.stderr.write(
                    "Could not determine size for submessage %s, using default %d\n"
    ...
    def encoded_size(self, dependencies):
        ...
        if self.pbtype in ["MESSAGE", "MSG_W_CB"]:
            ...
            raise RuntimeError(
                "Could not determine encoded size for %s.%s"
                % (self.struct_name, self.name)
            )
        else:
    ...
    def requires_custom_field_callback(self):
        return (
            self.allocation == "CALLBACK" and self.callback_datatype != "pb_callback_t"
        )
...
class ExtensionField(Field):
    ...
    def extension_decl(self):
        ...
        if self.skip:
            msg = (
                '/* Extension field %s was skipped because only "optional"\n'
                % self.fullname
            )
...
class OneOf(Field):
    ...
    def add_field(self, field):
        ...
        if self.sort_by_tag:
            self.fields.sort()
'DESCRIPTOR', 'DS_1', 'DS_2', 'DS_4', 'DS_8', 'DS_AUTO', 'DescriptorSize', 'FT_CALLBACK', 'FT_DEFAULT', 'FT_IGNORE', 'FT_INLINE', 'FT_POINTER', 'FT_STATIC', 'FieldType', 'IS_16', 'IS_32', 'IS_64', 'IS_8', 'IS_DEFAULT', 'IntSize', 'M_FLATTEN', 'M_NONE', 'M_PACKAGE_INITIALS', 'M_STRIP_PACKAGE', 'NANOPB_ENUMOPT_FIELD_NUMBER', 'NANOPB_FIELD_NUMBER', 'NANOPB_FILEOPT_FIELD_NUMBER', 'NANOPB_MSGOPT_FIELD_NUMBER', 'NanoPBOptions', 'TypenameMangling', '_DESCRIPTORSIZE', '_FIELDTYPE', '_INTSIZE', '_NANOPBOPTIONS', '_TYPENAMEMANGLING', '__builtins__', '__cached__', '__doc__', '__file__', '__loader__', '__name__', '__package__', '__spec__', '_builder', '_descriptor', '_descriptor_pool', '_globals', '_runtime_version', '_sym_db', '_symbol_database', 'google_dot_protobuf_dot_descriptor__pb2', 'nanopb', 'nanopb_enumopt', 'nanopb_fileopt', 'nanopb_msgopt']Writing to /home/ros2_ws/build/protolink/nanopb_gen/platformio/protolink_msgs/proto/std_msgs__String.pb.h and /home/ros2_ws/build/protolink/nanopb_gen/platformio/protolink_msgs/proto/std_msgs__String.pb.c
8['DESCRIPTOR', 'DS_1', 'DS_2', 'DS_4', 'DS_8', 'DS_AUTO', 'DescriptorSize', 'FT_CALLBACK', 'FT_DEFAULT', 'FT_IGNORE', 'FT_INLINE', 'FT_POINTER', 'FT_STATIC', 'FieldType', 'IS_16', 'IS_32', 'IS_64', 'IS_8', 'IS_DEFAULT', 'IntSize', 'M_FLATTEN', 'M_NONE', 'M_PACKAGE_INITIALS', 'M_STRIP_PACKAGE', 'NANOPB_ENUMOPT_FIELD_NUMBER', 'NANOPB_FIELD_NUMBER', 'NANOPB_FILEOPT_FIELD_NUMBER', 'NANOPB_MSGOPT_FIELD_NUMBER', 'NanoPBOptions', 'TypenameMangling', '_DESCRIPTORSIZE', '_FIELDTYPE', '_INTSIZE', '_NANOPBOPTIONS', '_TYPENAMEMANGLING', '__builtins__', '__cached__', '__doc__', '__file__', '__loader__', '__name__', '__package__', '__spec__', '_builder', '_descriptor', '_descriptor_pool', '_globals', '_runtime_version', '_sym_db', '_symbol_database', 'google_dot_protobuf_dot_descriptor__pb2', 'nanopb', 'nanopb_enumopt', 'nanopb_fileopt', 'nanopb_msgopt']Writing to /home/ros2_ws/build/protolink/nanopb_gen/platformio/protolink_msgs/proto/std_msgs__Header.pb.h and /home/ros2_ws/build/protolink/nanopb_gen/platformio/protolink_msgs/proto/std_msgs__Header.pb.c
8['DESCRIPTOR', 'DS_1', 'DS_2', 'DS_4', 'DS_8', 'DS_AUTO', 'DescriptorSize', 'FT_CALLBACK', 'FT_DEFAULT', 'FT_IGNORE', 'FT_INLINE', 'FT_POINTER', 'FT_STATIC', 'FieldType', 'IS_16', 'IS_32', 'IS_64', 'IS_8', 'IS_DEFAULT', 'IntSize', 'M_FLATTEN', 'M_NONE', 'M_PACKAGE_INITIALS', 'M_STRIP_PACKAGE', 'NANOPB_ENUMOPT_FIELD_NUMBER', 'NANOPB_FIELD_NUMBER', 'NANOPB_FILEOPT_FIELD_NUMBER', 'NANOPB_MSGOPT_FIELD_NUMBER', 'NanoPBOptions', 'TypenameMangling', '_DESCRIPTORSIZE', '_FIELDTYPE', '_INTSIZE', '_NANOPBOPTIONS', '_TYPENAMEMANGLING', '__builtins__', '__cached__', '__doc__', '__file__', '__loader__', '__name__', '__package__', '__spec__', '_builder', '_descriptor', '_descriptor_pool', '_globals', '_runtime_version', '_sym_db', '_symbol_database', 'google_dot_protobuf_dot_descriptor__pb2', 'nanopb', 'nanopb_enumopt', 'nanopb_fileopt', 'nanopb_msgopt']Writing to /home/ros2_ws/build/protolink/nanopb_gen/platformio/protolink_msgs/proto/std_msgs__MultiArrayLayout.pb.h and /home/ros2_ws/build/protolink/nanopb_gen/platformio/protolink_msgs/proto/std_msgs__MultiArrayLayout.pb.c
8['DESCRIPTOR', 'DS_1', 'DS_2', 'DS_4', 'DS_8', 'DS_AUTO', 'DescriptorSize', 'FT_CALLBACK', 'FT_DEFAULT', 'FT_IGNORE', 'FT_INLINE', 'FT_POINTER', 'FT_STATIC', 'FieldType', 'IS_16', 'IS_32', 'IS_64', 'IS_8', 'IS_DEFAULT', 'IntSize', 'M_FLATTEN', 'M_NONE', 'M_PACKAGE_INITIALS', 'M_STRIP_PACKAGE', 'NANOPB_ENUMOPT_FIELD_NUMBER', 'NANOPB_FIELD_NUMBER', 'NANOPB_FILEOPT_FIELD_NUMBER', 'NANOPB_MSGOPT_FIELD_NUMBER', 'NanoPBOptions', 'TypenameMangling', '_DESCRIPTORSIZE', '_FIELDTYPE', '_INTSIZE', '_NANOPBOPTIONS', '_TYPENAMEMANGLING', '__builtins__', '__cached__', '__doc__', '__file__', '__loader__', '__name__', '__package__', '__spec__', '_builder', '_descriptor', '_descriptor_pool', '_globals', '_runtime_version', '_sym_db', '_symbol_database', 'google_dot_protobuf_dot_des
    ...
    def __str__(self):
        ...
        if self.fields:
            ...
            if self.anonymous:
                ...
                result += (
                    "    union "
                    + Globals.naming_style.union_name(self.struct_name + self.name)
                    + " {\n"
                )
    ...
    def get_initializer(self, null_init):
        if self.has_msg_cb:
            return (
                "{{NULL}, NULL}, 0, {" + self.fields[0].get_initializer(null_init) + "}"
            )
        else:
    ...
    def encoded_size(self, dependencies):
        ...
        if len(dynamic_sizes) == 1:
            ...
            union_def = "union %s {%s};\n" % (
                union_name,
                " ".join("char f%d[%s];" % (k, s) for k, s in dynamic_sizes.items()),
            )
            required_defs = list(
                itertools.chain.from_iterable(
                    s.required_defines for k, s in dynamic_sizes.items()
                )
            )
            return EncodedSize(
                0, ["sizeof(union %s)" % union_name], [union_def], required_defs
            )
...
class Message(ProtoElement):
    ...
    def load_fields(self, desc, message_options):
        ...
        for index, f in enumerate(desc.field):
            ...
            field = Field(
                ...
                f,
                field_options,
                self.element_path + (ProtoElement.FIELD, index),
                self.comments,
            )
        ...
        if len(desc.extension_range) > 0:
            field_options = get_nanopb_suboptions(
                desc, message_options, self.name + "extensions"
            )
            range_start = min([r.start for r in desc.extension_range])
            if field_options.type != nanopb_pb2.FT_IGNORE:
                self.fields.append(
                    ExtensionRange(self.name, range_start, field_options)
                )
    ...
    def fields_declaration(self, dependencies):
        ...
        if has_callbacks:
            if self.callback_function != "pb_default_field_callback":
                result += (
                    "extern bool %s(pb_istream_t *istream, pb_ostream_t *ostream, const pb_field_t *field);\n"
        ...
        if defval:
            hexcoded = "".join(
                "\\x%02x" % ord(defval[i : i + 1]) for i in range(len(defval))
            )
    ...
    def fields_declaration_cpp_lookup(self, local_defines):
        ...
        result += (
            "    static PB_INLINE_CONSTEXPR const pb_size_t fields_array_length = %d;\n"
    ...
    def required_descriptor_width(self, dependencies):
        ...
        if max_arraysize > 0xFFFF:
            ...
            max_tag > 0x3FF
            or max_offset > 0xFFFF
            or max_arraysize > 0x0FFF
            or max_datasize > 0x0FFF
        ):
            return 4
    ...
    def default_value(self, dependencies):
        ...
        for field in reversed(list(optional_only.field)):
            ...
            if parsed_field is None or parsed_field.allocation != "STATIC":
                ...
                field.label == FieldD.LABEL_REPEATED
                or field.type == FieldD.TYPE_MESSAGE
            ):
                optional_only.field.remove(field)
                ...
                try:
                    ...
                    raise Exception(
                        "Could not find enum type %s while generating default values for %s.\n"
                        % (enumname, self.name)
                ...
                if not isinstance(enumtype, Enum):
                    raise Exception(
                        "Expected enum type as %s, got %s" % (enumname, repr(enumtype))
                    )

                if field.HasField("default_value"):
                    defvals = [
                        v
                        for n, v in enumtype.values
                        if n.parts[-1] == field.default_value
                    ]
                else:
...
def sort_dependencies(messages):
    ...
    while remaining:
        for candidate in remaining:
            ...
            sys.stderr.write(
                "Circular dependency in messages: "
                + ", ".join(remainset)
                + " (consider changing to FT_POINTER or FT_CALLBACK)\n"
            )
...
class MangleNames:
    ...
    def create_name(self, names):
        ...
        if str(names) not in self.name_mapping:
            ...
            if str(new_name) in self.reverse_name_mapping:
                sys.stderr.write(
                    "Warning: Duplicate name with mangle_names=%s: %s and %s map to %s\n"
                    % (
                        self.mangle_names,
                        self.reverse_name_mapping[str(new_name)],
                        names,
                        new_name,
                    )
                )
            ...
            if styled_name != unmangled_styled_name:
                ...
                if (
                    str(new_name) != unmangled_styled_name
                    and str(new_name) != styled_name
                ):
                ...
                if styled_name == str(new_name):
                    self.reverse_name_mapping[str(self.canonical_base + names)] = (
                        unmangled_styled_name
                    )
                else:
    ...
    def mangle_field_typename(self, typename):
        ...
        if self.strip_prefix is not None and typename.startswith(self.strip_prefix):
            if self.replacement_prefix is not None:
                return (
                    "." + self.replacement_prefix + typename[len(self.strip_prefix) :]
                )
            else:
...
class ProtoFile:
    ...
    def parse(self):
        ...
        self.comment_locations = {
            ...
            or location.leading_detached_comments
        ...
        for names, message, comment_path in iterate_messages(
            self.fdesc, self.manglenames.flatten
        ):
            ...
            if check_recursive_dependencies(msgobject, self.messages):
                ...
                sys.stderr.write(
                    "Breaking circular dependency at message %s by converting to %s\n"
        ...
        for names, extension in iterate_extensions(
            self.fdesc, self.manglenames.flatten
        ):
    ...
    def generate_header(self, includes, headername, options):
        ...
        for incfile in includes:
            ...
            yield options.genformat % (
                noext + options.extension + options.header_extension
            )
            yield "\n"
        ...
        if self.messages:
            ...
            for msg in self.messages:
                ...
                if unmangledName:
                    ...
                    self.manglenames.reverse_name_mapping[identifier] = (
                        unmangledIdentifier
                    )
            for msg in self.messages:
            ...
            for msg in self.messages:
                yield (
                    "extern const pb_msgdesc_t %s_msg;\n"
            ...
            local_defines = [
                identifier for identifier, msize in messagesizes if msize is not None
            ]
            ...
            max_messagesize = max(
                messagesizes,
                key=lambda messagesize: messagesize[1].value if messagesize[1] else 0,
            )
                ...
                if msize is not None:
                    ...
                    if identifier == max_messagesize[0]:
                        guards[cpp_guard].add(
                            "#define %-40s %s"
                            % (
                                Globals.naming_style.define_name(symbol + "_MAX_SIZE"),
                                Globals.naming_style.define_name(identifier),
                            )
                        )
        ...
        pairs = [
            x
            for x in self.manglenames.reverse_name_mapping.items()
            if str(x[0]) != str(x[1])
        ]
        if pairs:
    ...
    def generate_source(self, headername, options):
        ...
        if exceeds_64kB:
            yield (
                "\n/* The following messages exceed 64kB in size: "
                + ", ".join(exceeds_64kB)
                + " */\n"
            )
        ...
        if self.messages:
            ...
            if largest_count > 64:
                ...
                yield (
                    "#error Properly detecting missing required fields in %s requires \\\n"
                    % largest_msg.name
                )
                yield (
                    "       setting PB_MAX_REQUIRED_FIELDS to %d or more.\n"
                    % largest_count
                )
                yield "#endif\n"
...
def parse_custom_style(option, opt_str, value, parser):
    ...
    if len(parts) != 2 or not all(len(part) > 0 for part in parts):
        raise OptionValueError(
            "Invalid value for %s, must be in the form %s: %r"
...
def process_cmdline(args, is_plugin):
    ...
    if options.custom_style:
        ...
        class InheritNamingStyle(
            custom_class, NamingStyleC if options.c_style else NamingStyle
        ):
    ...
    if options.verbose:
        ...
        sys.stderr.write(
            "Google Python protobuf library imported from %s, version %s\n"
...
def process_file(filename, fdesc, options, other_files={}):
    ...
    excludes = (
        ["nanopb.proto", "google/protobuf/descriptor.proto"]
        + options.exclude
        + list(f.file_options.exclude)
    )
